...
def rand_poses(
    ...
    if theta_range != (0.0, 180.0):
        warnings.warn(
            "theta_range is not (0.0, 180.0) in rand_poses\n Will use (0.0, 180.0) instead"
        )
...
def rand_modal_poses(
    ...
    if overhead_flag:
        ...
        thetas = (
            ...
            * (theta_range_overhead[1] - theta_range_overhead[0])
        ...
        phis = rand_pertubations + torch.from_numpy(
            phi_mods[np.random.randint(0, 4, size)]
        ).to(device)
        thetas = (
            torch.rand(size, device=device) * (theta_range[1] - theta_range[0])
            + theta_range[0]
...
def circle_poses(
    device, radius=1.25, theta=60.0, phi=0.0, angle_overhead=30.0, angle_front=60.0
):
...
class MultiviewDataset:
    def __init__(self, cfg: RenderConfig, device):
        ...
        alternate_lists = (
            lambda l: [l[0]]
            + [i for j in zip(l[1 : size // 2], l[-1 : size // 2 : -1]) for i in j]
            + [l[size // 2]]
        )
    ...
    def collate(self, index):
        ...
        dirs, thetas, phis, radius = circle_poses(
            self.device,
            radius=radius,
            theta=theta,
            phi=phi,
        ...
        data = {"dir": dirs, "theta": thetas, "phi": phis, "radius": radius}
