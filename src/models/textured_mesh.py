...
def build_cotan_laplacian_torch(
    points_tensor: torch.Tensor, tris_tensor: torch.Tensor
) -> np.ndarray:
    ...
    vals = np.concatenate(
        (wab, wab, -wab, -wab, wbc, wbc, -wbc, -wbc, wca, wca, -wca, -wca), axis=0
    )
    L = sparse.coo_matrix(
        (vals, (rows, cols)), shape=(points.shape[1], points.shape[1]), dtype=float
    ).tocsc()
    return L
...
def choose_multi_modal(n: int, k: int):
    ...
    for i in range(n_intervals):
        ...
        chosen_numbers.append(
            np.random.choice(current_interval_length) + i * interval_length
        )
...
class TexturedMeshModel(nn.Module):
    def __init__(
        ...
        if self.opt.reference_texture:
            ...
            change_mask = (
                (base_texture.to(self.device) - self.texture_img).abs().sum(axis=1)
                > 0.1
            ).float()
            with torch.no_grad():
    ...
    def eigens(self, k: int, e: float) -> (torch.Tensor, torch.Tensor):
        if self._eigenvalues is None or self._eigenvectors is None:
            ...
            self._eigenvalues, self._eigenvectors = self._eigenvalues.to(
                self.device
            ), self._eigenvectors.to(self.device)

        return self._eigenvalues, self._eigenvectors

    @staticmethod
    def normalize_vertices(
        vertices: torch.Tensor, mesh_scale: float = 1.0, dy: float = 0.0
    ) -> torch.Tensor:
    ...
    def spectral_augmentations(self, vertices: torch.Tensor) -> torch.Tensor:
        ...
        basis_functions /= (
            basis_functions.max(dim=-1)[0][:, None]
            - basis_functions.min(dim=-1)[0][:, None]
        )
        ...
        signs = (
            (torch.rand(len(chosen_basis_function), device=self.device) > 0.5).float()
            - 0.5
        ) * 2
        ...
        return self.normalize_vertices(
            deformed_v, mesh_scale=self.mesh_scale, dy=self.dy
        )

    def axis_augmentations(
        self,
        vertices: torch.Tensor,
        stretch_factor: float = 1.6,
        squish_factor: float = 0.7,
    ):
    ...
    def init_paint(self, num_backgrounds=1):
        ...
        background_sphere_colors = nn.Parameter(
            modulated_init_background_bases_latent.cuda()
        )
        ...
        if self.initial_texture_path is not None:
            ...
            texture = (
                torch.ones(
                    1, 3, self.texture_resolution, self.texture_resolution
                ).cuda()
    ...
    def change_default_to_median(self):
        diff = (
            (
                self.texture_img
                - torch.tensor(self.default_color).view(1, 3, 1, 1).to(self.device)
            )
            .abs()
            .sum(axis=1)
        )
        ...
        median_color = (
            self.texture_img[0, :]
            .reshape(3, -1)[:, default_mask.flatten() == 0]
            .mean(axis=1)
        )
        with torch.no_grad():
            self.texture_img.reshape(3, -1)[
                :, default_mask.flatten() == 1
            ] = median_color.reshape(-1, 1)
    ...
    def init_texture_map(self):
        ...
        if (
            ...
            run_xatlas = not (
                vt.shape[0] == self.mesh.vertices.shape[0]
                and ft.shape[0] == self.mesh.faces.shape[0]
            )
        ...
        if run_xatlas:
            logger.info(
                "running xatlas to unwrap UVs for mesh: v={v_np.shape} f={f_np.shape}"
            )
            # unwrap uvs
            import xatlas
    ...
    def render(
        ...
        if use_render_back:
            ...
            if background is None:
                ...
                pred_back = torch.ones_like(pred_features) * background.reshape(
                    1, 3, 1, 1
                )
            else:
