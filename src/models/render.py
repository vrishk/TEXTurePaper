...
class Renderer:
    ...
    def render_single_view_texture(
        self,
        verts,
        faces,
        uv_face_attr,
        ...
        if render_cache is None:
            ...
            face_normals, uv_features, face_idx, depth_map = (
                render_cache["face_normals"],
                render_cache["uv_features"],
                render_cache["face_idx"],
                render_cache["depth_map"],
            )
        mask = (face_idx > -1).float()[..., None]

        image_features = kal.render.mesh.texture_mapping(
            uv_features, texture_map, mode=self.interpolation_mode
        )
        ...
        return (
            ...
            mask.permute(0, 3, 1, 2),
            depth_map.permute(0, 3, 1, 2),
            normals_image.permute(0, 3, 1, 2),
            render_cache,
        )

    def project_uv_single_view(
        self,
        verts,
        faces,
        uv_face_attr,
    ...
    def project_single_view(
        self, verts, faces, elev=0, azim=0, radius=2, look_at_height=0.0
    ):
